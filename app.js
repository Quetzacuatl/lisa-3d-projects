...
// Limiet verhoogd naar 10mb voor Base64 geüploade foto's.
...
app.post('/admin/products/new', requireAdmin, async (req, res) => {
  const { title, description, price, image_front, image_side, image_back, video_link } = req.body;

  if (!title || !description || !image_front) {
    return res.status(400).send("Titel, beschrijving en de voor-foto zijn verplicht.");
  }

  try {
    await db.query(
      `INSERT INTO products (title, description, price, image_front, image_side, image_back, video_link)
    ...
    res.status(500).send("Fout bij opslaan in database.");
  }
});
...
app.post('/admin/products/:id/edit', requireAdmin, async (req, res) => {
  ...
  try {
    await db.query(
      `UPDATE products 
       SET title = $1, description = $2, price = $3, image_front = $4, image_side = $5, image_back = $6, video_link = $7 
       WHERE id = $8`,
      [
        title,
        description,
    ...
    res.status(500).send("Fout bij updaten in database.");
  }
});
